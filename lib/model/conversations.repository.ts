...
import { resolveCreatorAllowAllLanguages } from '@/lib/billing/creator-language-access';
import {
...
export async function createConversation(
  ...
  try {
    ...
    console.error('createConversation:ensurePublicUser', e);
  }

  const creatorAllowsAll = await resolveCreatorAllowAllLanguages();
  const preferredLanguage = creatorAllowsAll
    ? payload.preferred_language
    : clampLanguageToFree(payload.preferred_language);
  assertLanguageAllowed(preferredLanguage, creatorAllowsAll);
